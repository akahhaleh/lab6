...
function addProjectDetails(e) {
	...
	$.get("project/"+idNumber, addProjectTrial)

	console.log("User clicked on project " + idNumber);
}

function addProjectTrial(result) {
	console.log(result);

	var projectHTML = '<a href="#" class="thumbnail">'+'<img src="' 
						+ result['image'] +'" class = "detailsImage">'
						+ '<h4>'+result['title'] +'</h4><p>'+result['date']+'</p></a><h7>'
						+result['summary']+'</h7>';
...
function changeColors(result){
	...
	var returnedColors = result['colors'];
	
	var colors = returnedColors['hex'];
